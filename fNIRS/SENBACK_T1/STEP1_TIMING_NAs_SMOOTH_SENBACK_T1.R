...
SENBACK_T1_CSVS <- file.path("PRE_PROCESSED_UNMASKED_EXTRACTED\\SENBACK_T1\\")
...
SENBACK_TIME <- read.csv("R_TASK_CLEANING_SCRIPTS\\SENBACK_T1_STIM_TIME.csv")
...
output_folder <- "FINAL_CLEAN_TASK_DATA\\SENBACK_T1\\ALL_CHANNELS"
...
if (length(df_list) == 0) {
  ...
  for (df_name in df_list) {
    ...
    file_path <- file.path(output_folder, paste0("DORRY3_FNIRS_CHANNELS_", id, "_SENBACK_T1.csv"))
