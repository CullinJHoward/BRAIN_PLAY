...
# set working directory
if (POUNDTOWN == 1) {
  work_dir <- 'D:\\FNIRS\\FINAL_CLEAN_TASK_DATA\\SENBACK_T2\\'
} else {
  work_dir <- 'F:\\FNIRS\\FINAL_CLEAN_TASK_DATA\\SENBACK_T2\\'
}
...
csv_files <- list.files(path = input_folder, pattern = "_SENBACK_T2_PW.csv$", full.names = TRUE)
...
if (exists("final_results_df")) {
  colnames(final_results_df)[-which(colnames(final_results_df) == "ID")] <- 
    paste0(colnames(final_results_df)[-which(colnames(final_results_df) == "ID")], "_SBT2")
  print("Updated column names in final_results_df:")
  print(colnames(final_results_df))
  write.csv(final_results_df, "SENBACK_T2_GLM_RESULTS.csv", row.names = FALSE)
}
